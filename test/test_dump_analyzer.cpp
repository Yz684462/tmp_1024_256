...
protected:
    void SetUp() override {
        ...
        std::string test_dump_content = 
            ...
            "  1079d4:\t1101\t\taddi\tsp,sp,-32\n"
            "  1079d6:\tec06\t\tsd\ts0,24(sp)\n"
            "  1079d8:\t4781\t\tli\ta5,-16\n"
            "  1079da:\t6422\t\tld\ts0,8(a5)\n"
            "  1079dc:\t853a\t\tmv\ta0,a5\n"
            "  1079de:\t85be\t\tmv\ta5,a0\n"
            "  1079e2:\t00000097\t\tjalr\tra5,-16\n";
...
public:
    ...
    TEST_F(DumpAnalyzerTest, ParseInstrLine) {
        ...
        EXPECT_EQ(instr->operands, "s0,16(sp)");
        EXPECT_EQ(instr->instrlen, 4);
    ...
    TEST_F(DumpAnalyzerTest, SelectSnippet) {
        ...
        EXPECT_EQ(snippet.size(), 4);  // Should find 4 instructions
        ...
        EXPECT_EQ(snippet[3]->address, 0x1079d0);
        EXPECT_EQ(snippet[3]->opcode, "ret");
        
        // Test invalid range (should throw exception)
        std::pair<uint64_t, uint64_t> invalid_range = {0x1079cc, 0x1079d5};
        EXPECT_THROW(analyzer->select_snippet(invalid_range), std::runtime_error);
    }
    ...
    TEST_F(DumpAnalyzerTest, SelectFuncContent) {
        ...
        EXPECT_GT(func_content.size(), 0);  // Should find some instructions
        
        // Verify instructions are within function range
        for (const auto& instr : func_content) {
            EXPECT_GE(instr->address, 0x1079d2);
            EXPECT_LT(instr->address, 0x1079e2);
        }
        
        // Test with address not in any function
        std::vector<BinaryTranslation::Instruction*> empty_content = analyzer->select_func_content(0x999999);
        EXPECT_EQ(empty_content.size(), 0);
    }
};
